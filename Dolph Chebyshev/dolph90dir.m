...
for iter = 1 : iterations
    for i = 1 : Nelem
        ...
        value = (DodB - 13)^2; % objective function, here 13 is taken for better performance

        if abs(value)<abs(swarm(i,4)) %if cost is less than global minimum then update best parameters
            swarm(i,2) = swarm(i,1);
            swarm(i,4) = value;
        end
        if abs(value)<=4 %if cost is less than threshold then exit
            break;
        end
    end
    ...
    pause(.1)
    if abs(temp)<=4 %if global minimum is less than threshold then exit
        break;
    end
end
